...
do_test_group1() {
    ...
    mc_test_should_fail "$csi40name" 2 "$vinname0" $(mc_make_skip "$csi40name" 2 "$csi1n" $csi1p)
    mc_test_should_fail "$csi40name" 1 "$vinname0" $(mc_make_skip "$csi40name" 1 "$csi1n" $csi1p)
    mc_test_should_fail "$csi20name" 1 "$vinname0" $(mc_make_skip "$csi20name" 1 "$csi1n" $csi1p)
    ...
    echo "VIN2"
    mc_test_should_fail "$csi20name" 2 "$vinname2" $(mc_make_skip "$csi20name" 2 "$csi3n" $csi3p)
...
do_test_group2() {
    ...
    mc_test_should_fail "$csi41name" 2 "$vinname4" $(mc_make_skip "$csi41name" 2 "$csi5n" $csi5p)
    mc_test_should_fail "$csi41name" 1 "$vinname4" $(mc_make_skip "$csi41name" 1 "$csi5n" $csi5p)
    mc_test_should_fail "$csi20name" 1 "$vinname4" $(mc_make_skip "$csi20name" 1 "$csi5n" $csi5p)
    ...
    echo "VIN6"
    mc_test_should_fail "$csi20name" 2 "$vinname6" $(mc_make_skip "$csi20name" 2 "$csi7n" $csi7p)
